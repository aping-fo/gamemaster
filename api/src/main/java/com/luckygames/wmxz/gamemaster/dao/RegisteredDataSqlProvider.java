...
public class RegisteredDataSqlProvider {
    public String searchPage(CommonSearchQuery query) {
        StringBuilder sql = new StringBuilder("SELECT t2.datelist update_time,IFNULL(t1.equipment_count,0)  equipment_count,IFNULL(t1.activation_count,0) activation_count,IFNULL(t1.loss_rate,0) loss_rate from registered_data t1 ")
                .append("RIGHT JOIN")
