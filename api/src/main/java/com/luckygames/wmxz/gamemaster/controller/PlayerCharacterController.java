...
public class PlayerCharacterController extends BaseController {
    ...
    public Response queryForbidden(PlayerCharacterSearchQuery query) {
        if (query.getPageNum() == null) {
            query.setPageNum(1);
        }
        Page<PlayerCharacter> characters = playerCharacterService.queryCharacterDailyReport(query);
    ...
    public Response info(PlayerCharacterSearchQuery query) {
        if (query.getPageNum() == null) {
            query.setPageNum(1);
        }
        Page<PlayerCharacter> characters = playerCharacterService.queryCharacterDailyReport(query);
    ...
    public Response baseInformationPlayer(Integer charId,Integer accountAuthority) {
        if(accountAuthority!=null){
            BaseInformation baseInformation = new BaseInformation();
            baseInformation.setCharId(charId);
            baseInformation.setAccountAuthority(accountAuthority);
            baseInformationService.updateByCharId(baseInformation);
            //TODO 同步账户权限
        }
        BaseInformation baseInformation = baseInformationService.searchById(charId);
